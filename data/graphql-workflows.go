...
// GraphqlWorkflowFiles fetches the name, path, and full text of every entry in a
// single directory. Fetching contents inline avoids the REST path's one call per
// file, which was the only unbounded API cost in a scan.
